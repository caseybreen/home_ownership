...
p <- p + geom_vline(xintercept =  as.Date(paste(1920, "-01-01", sep = "")), color = "black", linetype = "dashed", size = .7) + 
    annotate("text", x = as.Date(paste(1903, "-01-01", sep = "")), 
                y = 20, label = "1920 Census \n (Identify Sibs)", size= 3, 
                colour = "black")+ 
   annotate("segment", 
...
p <- p + geom_vline(xintercept =  as.Date(paste(1940, "-01-01", sep = "")), color = "black", linetype = "dashed", size = .7) + 
   annotate("text", x = as.Date(paste(1959, "-01-01", sep = "")), 
                y = 20, label = "1940 Census \n (Covariates)", size= 3, 
                colour = "black") +
   annotate("segment", 
...
p <- p + annotate("rect", xmin = as.Date(paste(2005, "-01-01", sep = "")), xmax = as.Date(paste(1975, "-01-01", sep = "")), ymin = 60, ymax = 110, alpha = .2, fill = "firebrick") + 
   annotate("text", x = as.Date(paste(1957, "-01-01", sep = "")), 
                y = 80, label = "Mortality Observation \n Window", size= 3, 
                colour = "firebrick") +
   annotate("segment", 
...
p <- p + geom_polygon(aes(x = positions$x, y = positions$y), fill = "NA", color = "black", linetype = "dotted", size= .7)
