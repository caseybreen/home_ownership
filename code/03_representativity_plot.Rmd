---
title: "Representativity Plots"
author: "Casey Breen"
---

Summary: Create plots assessing how representative our samples are of the general population. 
...
```{r}
sib_file <- fread(here("code/analysis_file/censoc_analysis_sibs.csv")) %>% 
...
census_1940_hh_vars_recode  <- census_1940_hh_vars %>% 
  ...
  filter(age %in% 25:35 & relate == 101)
...
dmf_characteristics_sibs <- census_1940_hh_vars_recode %>% 
  filter(histid %in% sib_file$histid) %>% 
  select(histid, race_recode, educ_level, sei_recode, marital_status, region_string, rural, homeown) %>%
  pivot_longer(-histid) %>% 
  group_by(name, value) %>%
  tally() %>%            
  mutate(prop = round(100*prop.table(n), 1)) %>% 
  rename(n_gen_sibs = n, prop_sibs = prop)


combined_characteristics <- census_characteristics %>% 
  inner_join(dmf_characteristics_conservative, by = c("name", "value")) %>% 
  inner_join(dmf_characteristics_sibs, by = c("name", "value")) %>% 
...
## save tables
table_s3 %>%
  gtsave("dmf_characteristics_table.tex", path = here("code/tables/")) 
```
