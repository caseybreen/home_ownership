...
model3 <- feols(death_age ~ ownership + educ_yrs + racec + urban | byear + occ + marst + statefip + serialp_1920 + eldest_brother, data = dmf_sibs)
model3.1 <- feols(death_age ~ ownership + educ_yrs + racec + urban | byear + occ + marst + statefip + serialp_1920, data = dmf_sibs)


## fit model 3 
results <- tidy(model3) %>% mutate(model = "Eldest Sibling Control") %>% 
  bind_rows(tidy(model3.1) %>% mutate(model = "No Eldest Born Control")) 
...
## fit model 3 
results <- tidy(model3_firstborn_homeowner) %>% mutate(model = "Eldest Born Homeowner") %>% 
...
summary(lm(incwage ~ as.factor(byear) + as.factor(eldest_brother), data = dmf_sibs_inc))

summary(lm(educ_yrs ~ as.factor(byear) + as.factor(eldest_brother), data = dmf_sibs_inc))
```
